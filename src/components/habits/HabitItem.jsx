...
export default function HabitItem({ habit, onCheckin, onUndoCheckin, onDelete }) {
    ...
    <>
      <div className={`flex items-center gap-4 p-4 rounded-xl border transition ${
        ...
        <div className="flex-1 min-w-0">
          <p className={`text-sm font-semibold ${isDoneToday ? 'line-through text-zinc-500' : 'text-white'}`}>
            {habit.name}
          </p>
          <div className="flex items-center gap-2 mt-1">
            ...
            <div className="flex gap-1 ml-1">
              {dots.map((done, i) => (
                <div
                  key={i}
                  className={`w-2 h-2 rounded-sm ${done ? 'bg-orange-500' : 'bg-zinc-700'}`}
                />
              ))}
            </div>
          </div>
